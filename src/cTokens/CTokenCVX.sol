...
import "./CTokenInterfacesCVX.sol";
import "../interfaces/ComptrollerInterface.sol";
import "../interfaces/ErrorReporter.sol";
 ...
 * @author Compound
 */
contract CTokenCVX is CTokenInterfaceCVX, Exponential, TokenErrorReporter {
    ...
    function initialize(
        ...
        symbol = symbol_;
        decimals = decimals_;
        cvxRewards = cvxRewards_;
        cvxToken = cvxToken_;
        cvxPoolID = cvxPoolId_;

        EIP20Interface(cvxToken).approve(cvxRewards, uint256(-1));
        addRewards();
    ...
    /**
     * @notice Copies rewards data directly from `cvxRewards` contract to `rewards` array of this contract
     */
    function addRewards() public {
        address mainPool = cvxRewards;
    ...
    /**
     * @notice Get length of `rewards` array
     * @return length of `rewards`
     */
    ...
    /**
     * @notice Get cToken balance of `_account`.
     * @param _account Address to fetch balance of
     * @return cToken balance of `_account`
     */
    function _getDepositedBalance(address _account)
        internal
        view
        returns (uint256)
    {
        ...
        return accountTokens[_account];
    }  

    /**
     * @notice Update rewards accounting for `_accounts` and optionally claim accrued rewards
     * @dev Called by `_checkpoint()` and `_checkpointAndClaim()`
     * @param _index index of reward to be updated/claimed. rewards[_index]
     * @param _accounts if _isClaim=true, `_accounts[0]` = reward claimant, `_accounts[1]` = reward recipient. if  _isClaim=false, both addresses updated.
     * @param _balances cToken balance of each address in `_accounts`
     * @param _supply Total supply of the cToken
     * @param _isClaim if true, claims reward tokens after updating rewards. if false, ONLY update rewards
     */
    function _calcRewardIntegral(
        ...
        uint256 bal = EIP20Interface(reward.reward_token).balanceOf(address(this));
        ...
        for (uint256 u = 0; u < _accounts.length; u++) {
            ...
            if (_isClaim || userI < reward.reward_integral) {
                if (_isClaim) {
                    uint256 receiveable = reward
                        .claimable_reward[_accounts[u]]
                        .add(
                            _balances[u]
                                .mul(uint256(reward.reward_integral).sub(userI))
                                .div(1e20)
                        );
                    if (receiveable > 0) {
                ...
                reward.reward_integral_for[_accounts[u]] = reward
                    .reward_integral;
            }
        }
    ...
    /**
     * @notice Claim CVX rewards accrued by this contract, then update all rewards for `_accounts`.
     * @dev Called by `userCheckpoint()` and `transferTokens()`
     * @param _accounts both of these addresses get their rewards checkpointed in `_calcRewardIntegral`
     */
    function _checkpoint(address[2] memory _accounts) internal {
        ...
        IRewardStaking(cvxRewards).getReward(address(this), true);
    ...
    /**
     * @notice Claim CVX rewards accrued by this contract. Then, update and claim all rewards for `_accounts[0]`, transferring the tokens to `_accounts[1]`.
     * @dev Called by `getReward()`
     * @param _accounts `_accounts[0]` = reward claimant, `_accounts[1]` = reward recipient 
     */
    ...
    /**
     * @notice Manually checkpoint rewards accounting for both addresses in `accounts`
     * @param _accounts The addresses of the accounts to checkpoint rewards for
     * @return Whether or not the checkpoint succeeded
     */
    ...
    /**
     * @notice Checkpoint and transfer reward tokens accrued by `_account`
     * @param _account The address of the account to claim rewards from
     */
    function getReward(address _account) public {
     ...
     /**
     * @notice Checkpoint rewards accrued by '_account` and transfer them to `forwardTo`
     * @param _account The address of the account to claim rewards from
     * @param _forwardTo The address of the account to receive tokens from claimed rewards
     */
    ...
    function transferTokens(
        ...
        // Checkpoint CVX rewards accrued by `src` and `dst`
        _checkpoint([src, dst]);
